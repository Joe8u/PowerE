...
def get_daily_template(year: int, month: int, day_type: str) -> pd.DataFrame:
    ...
    interpoliert aufs 15‑Minuten‑Raster und gibt DataFrame mit time-Index.
    """
    df = raw_df[
        (raw_df["Year"] == year)
        & (raw_df["Month"] == month)
...
if __name__ == "__main__":
    for month in range(1, 13):
        print(f"⏳ Erzeuge Monatsprofil {TARGET_YEAR}-{month:02d} …", end=" ")
        ...
        for day in range(1, days + 1):
            ...
            idx = pd.DatetimeIndex([
                pd.Timestamp.combine(date.date(), t.time()).tz_localize(TZ)
                for t in tpl.index
            ], freq="15min")
